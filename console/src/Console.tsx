...
export function Console() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      await murmur.mount(swarmRef.current);
      // Cohorts are pure computation, so the population can be drawn at rest before
      // anything is woken — an empty canvas on arrival reads as a failed load.
      try {
        const { cohorts, scenario } = await api.cohorts(20000);
        murmur.setCohorts(cohorts);
        setLog(
          `20,000 agents · ${cohorts.length} distinct situations · ` +
          `${scenario.souls_on_board.toLocaleString()} souls for ${scenario.seats_available.toLocaleString()} seats`,
        );
      } catch { /* the console still works without the resting field */ }
    })();
    ...
    <div className="console-shell">
      <header className="topbar">
        <div className="tagline">
          {mode === "swarm"
            ? "20,000 agents · one cloud per cohort · area is population"
            : "causal timeline · click an effect to compute its lightcone"}
        </div>
